...
static RoutineInfo * merge_tuple(HChar * line_input, RoutineInfo * curr,
        HChar * report, aprof_report * r, UInt * rid, Bool * invalid) {
    
    *invalid = False;
    
    if (VG_(strlen)(line_input) <= 0)
        return curr;
        ...
        *rid = id;
        ...
        if (curr->fn->mangled != NULL) {
            ASSERT(VG_(strcmp)(curr->fn->mangled, token) == 0,
                    "different mangled: %s <=> %s", curr->fn->mangled, token);
        } else {
        ...
        if (r->version == REPORT_VERSION && r->input_metric == RVMS) {
            ...
            ASSERT(rms_sqr <= ((double)rms) * ((double)rms) * ((double)occ),
            ...
            if (token != NULL) {
                ...
                ASSERT(rvms_syscall_sum <= rms*occ,
                ...
                ASSERT(rvms_thread_sum <= rms*occ,
                ...
                ASSERT(rvms_syscall_self <= rms*occ,
                        "invalid rvms syscall self: %s", line_orig);
                ...
                ASSERT(rvms_thread_self <= rms*occ,
        ...
        ASSERT(info_access->self_time_max >= info_access->self_time_min,
                "Invalid self min/max");
        ASSERT(info_access->self_time_sum >= info_access->self_time_max,
                "Invalid self sum");
        ...
        ASSERT(r->input_metric == RVMS,
                "Invalid report: rms but g - %s", line_orig);
...
static void check_rvms(double sum_rms, double sum_rvms, ULong num_rms,
        ULong num_rvms, HChar * name, HChar * report) {
    
    double ext_ratio = 0;
    
    ASSERT(sum_rms >= 0, "Invalid sum RMS - %s - %s", name, report);
    ASSERT(sum_rvms >= sum_rms,
            "RVMS should be bigger than RMS - %s - %s", name, report);
    ...
    if (ext_ratio == 0) {
        ASSERT(diff == 0,
                "Routine %s has external ratio %f and diff # %llu - %s", name, ext_ratio, diff, report);
    }
...
static void post_merge_consistency(aprof_report * r, HChar * report) {
    ...
    while (rtn != NULL) {
        ...
        if (consistency
                && VG_(strcmp)("dvector(long, long)", rtn->fn->name) == 0) {
            VG_(printf)("Function: %s\n", rtn->fn->name);
            VG_(printf)("\tSum thread input: %llu\n", induced_thread);
            VG_(printf)("\tSum syscall input: %llu\n", induced_syscall);
            VG_(printf)("\tSum rvms: %llu\n", sum_rvms);
        ...
        ASSERT(cumul_real > 0,
                "Invalid cumul real: %s:%s", rtn->fn->name, report);
        
        if (r->version == REPORT_VERSION && r->input_metric == RVMS) {
            
            check_rvms(sum_rms, sum_rvms, HT_count_nodes(rtn->distinct_rms),
                    HT_count_nodes(rtn->rvms_map), rtn->fn->name, report);
            ...
            ASSERT(sum_rms_distinct == sum_rms,
                    "Mismatch sum rms (tag v versus rvms in %s %s", rtn->fn->name, report)
...
static Bool merge_report(HChar * report, aprof_report * rep_data) {
    ...
    line[offset++] = '\0';
    current_routine = merge_tuple(line, current_routine, report, rep_data,
            &curr_rid, &invalid);
    ...
    *n = 0;
    while (1) {
        ...
        if (file->d_type == DT_REG
                && strcmp(".aprof", file->d_name + strlen(file->d_name) - 6)
                        == 0) {
...
static void print_diff(double a, double b) {
    ...
    if ((a == 0 && b == 0) || diff == 0)
        printf("[%+*.1f%%]", NUM_ALIGN, 0.0);
    else if (a == 0)
        printf("[" GREEN("%*s%%") "]", NUM_ALIGN, "+inf");
    else if (b == 0)
        printf("[" RED("%*s%%") "]", NUM_ALIGN, "-inf");
    else if (diff > 0)
        printf("[" GREEN("%+*.1f%%") "]", NUM_ALIGN, diff_p);
    else
        printf("[" RED("%+*.1f%%") "]", NUM_ALIGN, diff_p);
    
    printf("\n");
    
}

static void missing_routines(aprof_report * r, aprof_report * r2, UInt mode,
        Bool reverse) {
...
static void compare_distinct_rms(RoutineInfo * rtn, RoutineInfo * rtn2,
        aprof_report * r, aprof_report * r2, UInt mode) {
...
static void compare_report(aprof_report * r, aprof_report * r2, UInt mode) {
    ...
    printf("%*s | %*s | %*s | %*s\n", STR_ALIGN, "Report", NUM_ALIGN * 3,
            basename(logs[0]), NUM_ALIGN * 3, basename(logs[1]), NUM_ALIGN + 2,
            "diff(%)");
    ...
    printf("%*s | %*s | %*s | \n", STR_ALIGN, "", NUM_ALIGN * 3, "",
            NUM_ALIGN * 3, "");
    
    printf("%-*s", STR_ALIGN, "Version:");
    printf(" | %*u | %*u | \n", NUM_ALIGN * 3, r->version, NUM_ALIGN * 3,
            r2->version);
        ...
        while (info_access != NULL) {
            ...
            if (r->input_metric == RVMS) {
                ...
                fprintf(report,
                        ...
                        rtn_info->routine_id, info_access->key,
                        ...
                        info_access->self_time_sum, info_access->self_time_min,
                        info_access->self_time_max, info_access->self_sum_sqr,
...
static void clean_data(aprof_report * rep) {
    ...
    if (rep->routine_hash_table != NULL)
        HT_destruct(rep->routine_hash_table);
    if (rep->cmd != NULL)
        VG_(free)(rep->cmd);
    if (rep->app != NULL)
        VG_(free)(rep->app);
...
static void cmd_options(HChar * binary_name) {
    ...
    printf(
            "    -r         merge reports of different program's runs (different PID)\n");
    printf("               if no -t option is specified then this will not\n");
    printf("               merge reports of different threads.\n");
    
    printf(
            "    -t         merge reports of different threads of the same program's run (same PID)\n");
    
    printf("    -i         merge reports (no criteria)\n");
    
    printf(
            "    -c         compare two reports; required -a and -b arguments.\n");
    ...
    printf(
            "    -b <PATH>  compare report specified with -a with this report\n");
