...
public class Main {
    ...
    private static ArrayList<GraphPanel.Type> graph_order = null;
    ...
    public synchronized static ArrayList<GraphPanel.Type> getGraphOrder() {
        return graph_order;
    }

    public synchronized static void setGraphOrder(ArrayList<GraphPanel.Type> go) {
        graph_order = go;
        saveSettings();
    }
    ...
    private synchronized static void saveSettings() {
        try {
            ...
            for (int i = 0; i < graph_order.size(); i++) {
                out.print(graph_order.get(i).ordinal() + " ");
            }
            out.println("");
            ...
            if (rtn_name_mode == RtnNameMode.DEMANGLED) {
                out.println("0");
            } else {
                out.println("1");
            }
            ...
            if (rtn_cost_mode == Input.CostType.CUMULATIVE) {
                out.println("0");
            } else {
                out.println("1");
            }
            ...
            if (chart_cost_mode == Input.CostType.CUMULATIVE) {
                out.println("0");
            } else {
                out.println("1");
            }
        ...
        try {
            ...
            while ((line = in.readLine()) != null) {
                ...
                if (tag.equals("blacklist")) {
                    ...
                    graph_order = new ArrayList<GraphPanel.Type>();
                    while (tokenizer.hasMoreTokens()) {
            ...
            JOptionPane.showMessageDialog(null, "Settings file is corrupt;\nusing default settings");
            resetGraphOrder();
            saveSettings();
    ...
    private synchronized static void resetGraphOrder() {
        ...
        graph_order = new ArrayList<GraphPanel.Type>();
        graph_order.add(COST_PLOT); // cost plot
        graph_order.add(FREQ_PLOT); // frequency plot
        graph_order.add(RATIO_PLOT); // ratio plot
