...
static SizeT primes[N_HASH_PRIMES] = { 53UL, 97UL, 193UL, 389UL, 769UL, 1543UL,
...
void * HT_Next(HashTable * table) {
    ...
    /* See long comment on HT_Next prototype in pub_tool_hashtable.h.
     ...
     table whilst iterating over it, which is a bug. */vg_assert(table->iterOK);
    
    if (table->iterNode && table->iterNode->next) {
        table->iterNode = table->iterNode->next;
        if (table->iterNode == NULL)
            return NULL;
